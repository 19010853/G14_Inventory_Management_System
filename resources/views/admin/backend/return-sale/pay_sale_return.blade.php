   ...
   <div class="d-flex flex-column-fluid">
      <div class="container-fluid my-4">
         <div class="d-md-flex align-items-center justify-content-between">
            <h3 class="mb-0">Pay Sale Return</h3>
            <div class="text-end my-2 mt-md-0">
               <a class="btn btn-outline-primary" href="{{ route('due.sale.return') }}">Back</a>
            </div>
         </div>

         <div class="card">
            <div class="card-body">
               <form action="{{ route('update.sale.return.payment', $saleReturn->id) }}" method="post">
                  @csrf
                     ...
                     <div class="col-md-6">
                        <div class="card shadow-sm border-0">
                           ...
                           <div class="card-body p-4">
                              <div class="mb-3">
                                 <strong class="text-muted">Sale Return ID:</strong>
                                 <span class="ms-2">#{{ $saleReturn->id }}</span>
                              </div>
                              <div class="mb-3">
                                 <strong class="text-muted">Customer:</strong>
                                 <span class="ms-2">{{ $saleReturn->customer->name ?? 'N/A' }}</span>
                              </div>
                              <div class="mb-3">
                                 <strong class="text-muted">Warehouse:</strong>
                                 <span class="ms-2">{{ $saleReturn->warehouse->name ?? 'N/A' }}</span>
                              </div>
                              <div class="mb-3">
                                 <strong class="text-muted">Date:</strong>
                                 <span class="ms-2">{{ \Carbon\Carbon::parse($saleReturn->date)->format('Y-m-d') }}</span>
                              </div>
                              <div class="mb-3">
                                 <strong class="text-muted">Status:</strong>
                                 <span class="ms-2 badge bg-info">{{ $saleReturn->status }}</span>
                              </div>
                           </div>
                        </div>
                     </div>
                                 ...
                                 <table class="table border">
                                    <tbody>
                                       <tr>
                                          ...
                                          <td class="py-3 text-primary"><strong>${{ number_format($saleReturn->grand_total, 2) }}</strong></td>
                                       </tr>
                                       <tr>
                                          <td class="py-3"><strong>Paid Amount:</strong></td>
                                          <td class="py-3">${{ number_format($saleReturn->paid_amount, 2) }}</td>
                                       </tr>
                                       <tr>
                                          <td class="py-3"><strong>Full Paid:</strong></td>
                                          <td class="py-3">${{ number_format($saleReturn->full_paid ?? 0, 2) }}</td>
                                       </tr>
                                       <tr>
                                          <td class="py-3"><strong>Due Amount:</strong></td>
                                          <td class="py-3">
                                             <span class="badge bg-danger">${{ number_format($saleReturn->due_amount, 2) }}</span>
                                          </td>
                                       </tr>
                                    </tbody>
                                 </table>
                           ...
                           <div class="card-body p-4">
                              <div class="mb-3">
                                 ...
                                 <input type="number" 
                                        ...
                                        id="paid_amount" 
                                        class="form-control" 
                                        value="{{ $saleReturn->paid_amount }}" 
                                        step="0.01" 
                                        min="0" 
                                        max="{{ $saleReturn->grand_total }}"
                                        required>
                                 @error('paid_amount')
                                    <span class="text-danger">{{ $message }}</span>
                                 @enderror
                                 <small class="text-muted">Current due amount: ${{ number_format($saleReturn->due_amount, 2) }}</small>
                              </div>
...
document.addEventListener('DOMContentLoaded', function() {
   ...
   const grandTotal = {{ $saleReturn->grand_total }};
